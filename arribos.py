...
if response.status_code == 200:
    ...
    # Encuentra la tabla de arribos por su id
    tabla_arribos = soup.find("table", {"class": "scrollvuelos-main"})

    # Verifica si la tabla de arribos se encontró
    if tabla_arribos:
        # Itera a través de las filas de la tabla
        for fila in tabla_arribos.find("tbody", {"class": "listArribos longtable"}).find_all("tr", {"class": "popup"}):
        ...
        arribos_dict = {
            "Título": "Arribos a Jujuy",
            "Datos": datos_arribos
        }

        # Escribe los datos en un archivo JSON
        with open("arribos_jujuy.json", "w", encoding="utf-8") as json_file:
            json.dump(arribos_dict, json_file, ensure_ascii=False, indent=4)

        print("Los datos de arribos se han guardado en arribos_jujuy.json con el título 'Arribos a Jujuy'.")
    else:
        print("No se encontró la tabla de arribos en la página web.")
else:
